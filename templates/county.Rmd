...
v %>% 
  filter(area == params$area) %>% 
  select(date_end, adult_n, adult_pct) %>% 
  ...
  select(
    ...
    "Estimated percentage of adults reporting food insufficiency" = adult_pct,
    "Estimated number of adults reporting food insufficiency" = adult_n
  ) %>% 
  reactable(
    showSortable = TRUE,
    defaultColDef = 
...
In the most recent survey period that ended `r format(v$date_end, "%B %e")`, an estimated `r format(round(v$adult_pct, digits = 1), nsmall = 1)`% of `r params$area` adults, or approximately `r format(signif(v$adult_n, digits = 3), big.mark = ",")` adults, lived in households experiencing food insufficiency.
