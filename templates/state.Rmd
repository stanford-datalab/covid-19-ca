...
v %>% 
  ...
  mutate(
    ...
    adult_n = signif(adult_n, digits = 3),
    adult_pct = format(round(adult_pct, digits = 1), nsmall = 1)
  ) %>% 
  select(
    "Survey end date" = date_end, 
    "Percentage of adults reporting food insufficiency" = adult_pct,
    "Adults reporting food insufficiency" = adult_n
  ) %>% 
  reactable(
    showSortable = TRUE,
    defaultColDef = 
...
In the most recent survey period that ended `r month_day(v$date_end)`, an estimated `r format(round(v$adult_pct, digits = 1), nsmall = 1)`% of `r params$area` adults, or approximately `r format(signif(v$adult_n, digits = 3), big.mark = ",")` adults, lived in households with food insufficiency.

Using a model based upon data from the Current Population Survey Food Security Supplement, we estimate that in the most recent survey period, `r format(round(v$child_pct, digits = 1), nsmall = 1)`% of children, or approximately `r format(signif(v$child_n, digits = 3), big.mark = ",")` children, experienced food insufficiency.
...
ggplot(mapping = aes(unemployment_rate, food_insufficiency_pct)) +
  ...
  labs(
    title = "California unemployment and food insufficiency over time",
    x = "Unemployment rate",
    y = "Percentage of adults reporting food insufficiency",
    caption = 
      str_c(
...
The relationship between food insufficiency and unemployment that held between 2001 and 2018 appears to have changed during the pandemic. May 2020 experienced both high unemployment and high food insufficiency, but food insuffiency is much higher than past years would predict. 

Food insufficiency data for December 2019 is not yet available, so the plot does not show 2019. 
