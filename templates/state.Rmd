...
# Choropleth map
choropleth <- function(
    data, var, population, tooltip, legend_title, breaks = NULL
  ) {
  ...
  if (length(bins) != length(unique(bins))) {
    while ((length(bins) > 3) & (length(bins) != length(unique(bins)))) {
      bins <-
        Hmisc::wtd.quantile(
          values,
          weights = data %>% pull({{population}}),
          probs = seq(0, 1, length.out = length(bins) - 1)
        )
    }
  }
...
The data on this site is derived from the U.S. Census Bureau [Household Pulse Survey (HPS)](https://www.census.gov/householdpulsedata) and the U.S. Bureau of Labor Statistics [Local Area Unemployment Statistics (LAUS)](https://www.bls.gov/lau/home.htm). New HPS data is currently released every four weeks, and new LAUS data is released once a month. The most recent data used for this report are:
...
v <- 
  ...
  filter(date_end > hps_date_end - years(1))
...
v %>% 
  ...
  labs(
    ...
    caption = "Sources: 2020 May and later: Census Bureau, Household Pulse Survey.\n2019 Dec: Current Population Survey, Food Security Supplement."
  )
```
...
The percentages and numbers of adults in the above table are based upon those who answered the survey question on food insufficiency. Even if the percentage of adults reporting food insufficiency is the same in two surveys, the number of adults reporting food insufficiency in the two surveys could vary depending upon the number who answered the question.
...
v <- 
  hps %>% 
  filter(
    str_detect(variable, "^foodrsnrv[1-4]_special$"), !is.na(code)
  ) %>% 
  filter(date_end > hps_date_end - years(1))
...
v %>% 
  ...
  geom_line() +
  geom_point() +
  coord_cartesian(xlim = c(date_min, NA), ylim = c(0, NA)) +
  scale_x_date(
  ...
  labs(
    ...
    x = "Survey end date",
    y = "Percentage reporting reason",
    color = NULL,
    caption = "Source: Census Bureau, Household Pulse Survey."
  )
```

The above plot contains the reasons respondents gave for their household's food insufficiency. Respondents can choose more than one reason, so the percentages sum to greater than 100%. This question was first asked in the survey that ended `r format(series_start, format = "%Y %b %e")`.
...
<!--   labs( -->
<!--     title =  -->
<!--     "Do you or does anyone in your household receive benefits from the\nSupplemental Nutrition Assistance Program (SNAP) or the Food Stamp Program?", -->
<!--     x = "Survey end date", -->
<!--     y = "Percentage reporting Yes", -->
<!--     caption = "Source: Census Bureau, Household Pulse Survey." -->
<!--   ) -->
<!-- ``` -->

<!-- This question was first asked in the survey that ended `r format(series_start, format = "%Y %b %e")`. -->

### Supplemental Nutrition Assitance Program (SNAP) and Special Supplementary Nutrition Assistance Program for Women, Infants, and Children (WIC)

```{r}
v <-
  hps %>%
  filter(
    str_detect(variable, "^fdbenefit"),
    date_end > hps_date_end - years(1)
  ) %>% 
  ...
  ungroup() %>% 
  filter(code == 1) %>% 
  mutate(label = str_extract(response, "^\\w+"))
...
v %>%
  ggplot(aes(date_end, pct, color = label)) +
  geom_line() +
  geom_point() +
  coord_cartesian(xlim = c(date_min, NA), ylim = c(0, NA)) +
  scale_x_date(
  ...
  labs(
    title =
    "Do you or does anyone in your household receive benefits from (SNAP) or (WIC)?",
    x = "Survey end date",
    y = "Percentage reporting receipt",
    color = NULL,
    caption = "Source: Census Bureau, Household Pulse Survey."
  )
```
...
v %>% 
  ...
  labs(
    title = 
      "During the last 7 days, did you or anyone in your household get free groceries\nfrom a food pantry, food bank, church, or other place that provides free food?",
    x = "Survey end date",
...
years <- 
  v$date %>% 
  keep(~ lubridate::month(.) == 1)

year_labels <- 
  tibble(
    date = years,
    unemployment_rate = max(v$unemployment_rate),
    label = year(date)
  )

breaks <- 
  v$date %>% 
  keep(~ lubridate::month(.) %% 2 == 1)
  
v %>% 
  filter(area_type == "State") %>% 
  ggplot(aes(date, unemployment_rate)) +
  geom_vline(xintercept = years, color = "grey60") +
  geom_text(aes(label = label), data = year_labels, hjust = -0.1) +
  geom_line() +
  geom_point() +
  coord_cartesian(ylim = c(0, NA)) +
  scale_x_date(
...
<!--     caption =  -->
<!--       str_c( -->
<!--         "Sources: 2020 May and later: Census Bureau, Household Pulse Survey.\n", -->
<!--         "2019 Dec and earlier: Current Population Survey, Food Security Supplement.\n", -->
<!--         "Bureau of Labor Statistics, Local Area Unemployment Statistics." -->
<!--       ) -->
<!--   ) -->
<!-- ``` -->
