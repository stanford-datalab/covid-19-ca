...
file_cps_fss <- here::here("data/cps_fss.csv")
  # Population data
...
# Food insufficiency from most recent CPS Food Security Supplement
cps_fss_food_insufficiency <- 
  read_csv(file_cps_fss) %>% 
  filter(
    area == params$area,
    year == min(2019, max(year)),
    variable == "fsfoods",
    str_detect(response, "(sometimes|often) not enough to eat")
  ) %>% 
  group_by(year) %>% 
...
v %>% 
  ...
  geom_hline(yintercept = cps_fss_food_insufficiency$pct, color = "blue") +
  geom_line() +
  geom_point() +
  annotate(
    geom = "text",
    x = min(v$date_end),
    y = cps_fss_food_insufficiency$pct,
    label = 
      str_glue("Food insufficiency in Dec {cps_fss_food_insufficiency$year}"),
    hjust = 0,
    vjust = 1.5
  ) +
  scale_x_date(
...
In the most recent survey period that ended `r month_day(v$date_end)`, an estimated `r format(round(v$pct, digits = 1), nsmall = 1)`% of `r params$area` adults, or approximately `r format(signif(v$n, digits = 3), big.mark = ",")` adults, lived in households with food insufficiency.
...
v %>% 
  choropleth(
    ...
    tooltip = 
      "<strong>{area}</strong><br/>Food insufficiency: {format(pct, trim = TRUE, nsmall = 1)}%",
...
The Household Pulse Survey does not provide data at the county level, so these food insuffiency numbers are estimates from a model. Our model uses the food insuffiency rate of `r data_params$state` and the county unemployment rates to estimate food insufficiency for each county.

The state-level food insufficiency data is updated more frequently than the county unemployment rates, and each month's unemployment data comes out later than the food insufficiencey data. If unemployment data is not yet available for a month, the model uses the latest month for which data is available. 
